...
export default function CategoryDetailsPage() {
  ...
  async function getCategoryDetails(id) {
    try {
      ...
      const categoryDetails = await GET({
      ...
      <Container maxWidth="lg" className="py-5">
        ...
        {!loading && !error && (
          <>
              ...
              <div className={styles.panel}>
                ...
                <div className="mt-3">
                  ...
                  <h1 className={styles.title}>
                    {categoryData.title?.en || categoryData.title}
                  </h1>
                  {categoryData.title?.bn && (
                    <p className={styles.subtitle}>{categoryData.title.bn}</p>
                  )}
                </div>
                ...
                {categoryData.description && (
                  <div className={styles.block}>
                    ...
                    <p>
                      {categoryData.description?.en || categoryData.description}
                    </p>
                    {categoryData.description?.bn && (
                      <p>{categoryData.description.bn}</p>
                    )}
                  </div>
                )}
            ...
            {categoryData?.problem_types &&
              categoryData.problem_types.length > 0 && (
                <>
                  ...
                  <section className="row g-4">
                    {categoryData.problem_types.map((problemType) => (
                      <div
                        ...
                        key={problemType._id || problemType.id}
                      >
                        <Link
                        ...
                        >
                          <div className={styles.card}>
                            ...
                            {problemType.title?.bn && (
                              <p className={styles.cardTitleBn}>
                                {problemType.title.bn}
                              </p>
                            )}
                            {problemType.description && (
                              <p className={styles.cardDescription}>
                                {problemType.description?.en ||
                                  problemType.description}
                              </p>
                            )}
